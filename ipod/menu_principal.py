    ...
    def cargar_datos(self):
        ...
        try:
            ...
            # --- FAV ALBUMS ---
                ...
                print("Descargando álbumes guardados...")
                res = self.sp.current_user_saved_albums(limit=50)
                # OJO: Aquí la estructura es item['album']['name']
                nuevas = [{'nombre': i['album']['name'], 'uri': i['album']['uri']} for i in res['items']]
            ...
            # --- NEW RELEASES ---
                ...
                # Aquí la estructura es res['albums']['items']
                nuevas = [{'nombre': i['name'], 'uri': i['uri']} for i in res['albums']['items']]
            
            # --- ALBUMS (ARTIST) ---
            elif self.tipo_carga == 'artist_albums' and self.id_padre:
                print(f"Cargando álbumes del artista {self.id_padre}...")
            ...
            # --- SONGS (ALBUM) ---
                ...
                print(f"Cargando canciones del álbum {self.id_padre}...")
            ...
            # --- SONGS (PLAYLIST) ---
                ...
                # En playlist, la canción está dentro de 'track'
                nuevas = []
                for item in res['items']:
                    if item['track']: # A veces hay items vacíos
                        nuevas.append({'nombre': item['track']['name'], 'uri': item['track']['uri'], 'type': 'track'})
