...
def dibujar_header(pantalla, contenido, estado_play):
    ...
    text_time = _fuente_header_small_cache.render(time_str, ANTIALIASING, VERDE_SPOTIFY)
    ...
    if estado_play is True:
        ...
        pygame.draw.rect(pantalla, VERDE_SPOTIFY, (x_pause, y_pause, 3, 10))
        pygame.draw.rect(pantalla, VERDE_SPOTIFY, (x_pause + 6, y_pause, 3, 10))

    # CONTENIDO CENTRAL (Texto o Surface)
    if isinstance(contenido, str):
        # Es texto normal
        surf = _fuente_header_big_cache.render(contenido, ANTIALIASING, VERDE_SPOTIFY)
    else:
    ...
    # WIFI (Derecha)
    # Se establece el color del icono
    color_wifi = VERDE_SPOTIFY if HAY_CONEXION else (100, 100, 100)
    
    # Posición del icono
    x_wifi = 260
    y_wifi = ALTURA_HEADER//2 + 4
    # Punto base
    pygame.draw.circle(pantalla, color_wifi, (x_wifi + 6, y_wifi), 1)
    # Arco pequeño
    pygame.draw.arc(pantalla, color_wifi, (x_wifi + 2, y_wifi - 5, 9, 9), 1.57, 2.6, 2)
    pygame.draw.arc(pantalla, color_wifi, (x_wifi + 3, y_wifi - 5, 9, 9), 0.729, 1.57, 2)
    pygame.draw.line(pantalla, (0, 0, 0), (x_wifi + 9, y_wifi - 3), (x_wifi + 9, y_wifi - 3), 1)
    # Arco grande
    pygame.draw.arc(pantalla, color_wifi, (x_wifi - 1, y_wifi - 9, 14, 14), 1.57, 2.6, 2)
    pygame.draw.arc(pantalla, color_wifi, (x_wifi, y_wifi - 9, 14, 14), 0.729, 1.57, 2)
    pygame.draw.line(pantalla, color_wifi, (x_wifi + 11, y_wifi - 8), (x_wifi + 11, y_wifi - 8), 1)
    pygame.draw.line(pantalla, color_wifi, (x_wifi + 9, y_wifi - 9), (x_wifi + 9, y_wifi - 9), 1)

    # BATERÍA (Derecha) - Porcentaje
    # Placeholder: 100% (Más adelante leeremos el sistema real)
    bateria_pct = "100%" 
    text_bat = _fuente_header_small_cache.render(bateria_pct, ANTIALIASING, VERDE_SPOTIFY)
    rect_bat = text_bat.get_rect(midright=(ANCHO - 5, ALTURA_HEADER // 2))
