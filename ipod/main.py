...
menu_shows = MenuPantalla("Shows", [], sp, 'menu_shows')
            ...
            if isinstance(curr, MenuPantalla):
                ...
                if e.key == pygame.K_RETURN:
                    ...
                    # B) Es un elemento de datos de Spotify
                        ...
                        tipo = sel.get('type') # 'artist', 'album', 'playlist', 'track', 'show'
                        uri = sel['uri']
                        nombre = sel['nombre']
                        ...
                        # 1. Si pulso en "Podcasts" en el menú raíz -> Abre la lista de Shows
                        elif tipo == 'menu_shows':
                            # 'user_shows' activará el bloque nuevo en menu_screen
                            stack.append(MenuPantalla(nombre, [], sp, 'user_shows'))

                        elif tipo == 'show':
                            # Aquí SÍ necesitamos el ID para saber de qué show cargar los capítulos
                            # Fíjate que en menu_screen guardamos 'id' en el diccionario, úsalo aquí
                            el_id = res['id'] 
                            stack.append(MenuPantalla(nombre, [], sp, 'show_episodes', id_padre=el_id))
                        
                        # 3. Si pulso en un Episodio -> Reproduce
                        elif tipo == 'episode':
                            print(f"Reproduciendo: {nombre}")
                            try:
                                sp.start_playback(uris=[uri])
