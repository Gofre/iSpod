            ...
            # --- 2. SEARCH ---
                ...
                if e.key == pygame.K_RETURN:
                    ...
                    if res and res['tipo'] == 'item':
                        ...
                        if tipo == 'track':
                            try:
                                sp.start_playback(uris=[res['uri']])
                                stack.append(now_playing)
                            except: print("Error Playback Search")
                        elif tipo == 'artist':
                            stack.append(MenuPantalla(res['nombre'], [], sp, 'artist_albums', id_padre=res['uri']))
                        elif tipo == 'album':
                            stack.append(MenuPantalla(res['nombre'], [], sp, 'album_tracks', id_padre=res['uri']))
                        elif tipo == 'playlist':
                            stack.append(MenuPantalla(res['nombre'], [], sp, 'playlist_tracks', id_padre=res['uri']))
